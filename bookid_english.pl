...
$sth_drop=$dbh->prepare("DROP TABLE IF EXISTS english_books_list");
$sth_drop->execute();
$sth_drop->finish();
...
sub insert_to_db()
{
	...
	$sth2=$dbh->prepare("insert into english_books_list values('$book_id','$level','$title','$authid','$authors','$page','$page_end','$edition','$volume','$part','$type','$year','$month','0')");
	$sth2->execute();
	$sth2->finish();
}
