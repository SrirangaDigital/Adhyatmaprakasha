...
$sth_drop=$dbh->prepare("DROP TABLE IF EXISTS sanskrit_book_toc");
$sth_drop->execute();
$sth_drop->finish();
...
sub insert_to_db()
{
	...
	$sth2=$dbh->prepare("insert into sanskrit_book_toc values('$book_id','$btitle','$level','$title','$page','$type','0')");
	$sth2->execute();
	$sth2->finish();
}
