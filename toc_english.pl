...
open(IN,"apk_english_toc.xml") or die "can't open apk_english_toc.xml\n";

my $dbh=DBI->connect("DBI:mysql:database=$db;host=$host","$usr","$pwd");

$sth1=$dbh->prepare("CREATE TABLE english_book_toc(
book_id varchar(4),
btitle varchar(2000),
level int(2),
title varchar(10000),
...
sub insert_to_db()
{
	...
	$sth2=$dbh->prepare("insert into english_book_toc values('$book_id','$btitle','$level','$title','$page','')");
	$sth2->execute();
	$sth2->finish();
}
